<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:92:"/www/wwwroot/www.fdadeal.com/public/../application/index/view/wanlshop/order/wholesale1.html";i:1637072588;s:72:"/www/wwwroot/www.fdadeal.com/application/index/view/layout/wanlshop.html";i:1636510733;}*/ ?>
<!DOCTYPE html>
...
<div class="row">
    <label class="control-label col-sm-2" style="color:red;font-size:22px;">支払いパスワード：<p style="font-size: 13px;
    color: black;">[設定/変更なし]セラーセンター>>個人情報>>支払いパスワードの変更に移動してください</p><input id="c-password" class="form-control" name="password" type="text" maxlength="6" onkeyup="value=value.replace(/[^\d\.]/g,'')" placeholder="6桁の取引パスワードを入力してください" /></label> 
	<!--<div class="pull-left"><a href="<?php echo url('index/wanlshop.user/changepwd2'); ?>" class="btn btn-primary"><i class="fa fa-user"></i> 修改支付密碼</a></div>-->
    <p> </p>
	<div class="col-md-12">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<p><span>注文番号：</span><small><?php echo $row['order_no']; ?></small> <span style="margin-left: 30px;">買い手：</span><small><?php echo $row['user']['nickname']; ?></small><strong></strong></p>
				...
				<table class="table table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<td><?php echo $vo['wholesale_price']; ?></td>
					...
					<tfoot>
						...
						<tr>
							...
							<th colspan="9" style="text-align: right;">
								...
								<p class="ordertext">運送（<?php switch($row['freight_type']): case "0": ?>貨物の積み重ね<?php break; case "1": ?>最小決済<?php break; case "2": ?>最大決済<?php break; endswitch; ?>）：<samp class="text-red"><?php echo $row['pay']['freight_price']; ?> 円</samp> </p>

								<p class="ordertext">割引価格：<samp class="text-red"><?php echo $row['pay']['discount_price']; ?> 円</samp> </p>

								<p class="ordertext">実際の売上高：<samp class="text-red"><?php echo $row['pay']['price']; ?> 円</samp></p>

								<p class="ordertext">卸売価格：<samp class="text-red"><?php echo $row['pay']['wholesale_price']; ?> 円</samp></p>
								
								<p class="ordertext">注文利益：<samp class="text-red"><?php echo $row['pay']['price']-$row['pay']['wholesale_price']; ?> 円</samp></p>
				...
				<div style="color: #333;">
					...
					<p style="margin-bottom: 15px;">購入者の支払い方法：
					...
					<p style="margin-bottom: 15px;">取引状況：
					...
					<?php if($row['coupon_id'] != '0'): ?>
						...
						<p>ディスカウント：<?php echo $row['coupon']['name']; ?>

						</p>

					<?php endif; ?>

					<p>配送方法：<small><?php echo !empty($row['express_name'])?$row['express_name']: '配達の10日後にサプライヤーによって出荷され、買い手は異議を唱えませんシステムは自動的に受領を確認します'; ?></small></p>
					...
					<?php if(!(empty($row['remarks']) || (($row['remarks'] instanceof \think\Collection || $row['remarks'] instanceof \think\Paginator ) && $row['remarks']->isEmpty()))): ?>
						...
						<p>バイヤーメッセージ：<?php echo $row['remarks']; ?></small></p>
	...
	<div class="col-md-12">
		...
		<div class="panel panel-default">
			...
			<div class="panel-heading">お届け先の住所</div>
				...
				<table class="table table-striped">
					...
					<tbody>
						...
						<tr>
							...
							<td><?php if($row['wholesale_id']!=0){echo substr($row['address']['mobile'],0,2).'****'.substr($row['address']['mobile'],6,strlen($row['address']['mobile']));}else{
echo $row['address']['mobile'];
	...
	<div class="col-md-12">
		...
		<div class="panel panel-default">
			...
			<div class="panel-heading">配送状況</div>
				...
				<table class="table table-bordered">
					...
					<thead>
						...
						<tr>
							...
							<th class="text-center">
								...
								<div class="th-inner">宅配会社</div>

							</th>

							<th class="text-center">

								<div class="th-inner">宅配便番号</div>

							</th>

							<th class="text-center">

								<div class="th-inner">配送状況</div>

							</th>

							<th class="text-center">

								<div class="th-inner">ロジスティクス情報</div>

							</th>

							<th class="text-center">

								<div class="th-inner">更新時間</div>

							</th>

							<th class="text-center">

								<div class="th-inner">オペレーティング</div>
					...
					<tbody>
						...
						<tr>
							...
							<td><a href="javascript:;" class="btn btn-xs btn-info btn-selected kuaidisub" data-id="<?php echo $row['id']; ?>" data-toggle="tooltip" data-original-title="ロジスティクスを見る"><i class="fa fa-eye"></i></a></td>
...
<div class="hide layer-footer">
	<label class="control-label col-sm-2">卸売価格合計：<samp class="text-red"><?php echo $row['pay']['wholesale_price']; ?> 円</samp></label>
    <label class="control-label col-sm-2">残高：<samp class="text-red"><?php echo $user['money']; ?> 円</samp></label>
    
	<div class="col-xs-12 col-sm-8">
        <button type="submit" class="btn btn-success btn-embossed disabled"><?php echo __('OK'); ?>卸売り支払い</button>
