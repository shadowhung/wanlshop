<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:89:"/www/wwwroot/www.fdadeal.com/public/../application/index/view/wanlshop/order/invoice.html";i:1634747772;s:72:"/www/wwwroot/www.fdadeal.com/application/index/view/layout/wanlshop.html";i:1636510733;}*/ ?>
<!DOCTYPE html>
								...
								<div class="row">
									...
									<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
										...
										<div class="content">
											...
											<div id="print_html">	<link href="/assets/css/backend.css?v=1581391093" rel="stylesheet">	<style type="text/css">		h3{			margin-bottom: 40px;		}		#print_html{			font-size: 11px!important;			overflow: hidden;		}		.table-head>tbody>tr>th,		.table-head>tbody>tr>td {			border: 0px;			padding: 5px;			line-height: 1;		}		.table>tbody>tr>th,		.table>tbody>tr>td {			font-size: 12px;		}		.table>thead>tr>th{			font-size: 12px;		}		.wanlpay span{			margin-left: 10px;		}	</style>	<?php if(is_array($row) || $row instanceof \think\Collection || $row instanceof \think\Paginator): $i = 0; $__LIST__ = $row;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?>		<h3 align="center">商品請求書</h3>		<table class="table table-head">			<tbody>				<tr>					<td>ユーザー名：<?php echo $order['user']['username']; ?></td>					<td colspan="2">名前：<?php echo $order['address']['name']; ?></td>					<td style="text-align: right;">注文番号：<?php echo $order['order_no']; ?></td>				</tr>				<tr>					<td>住所：<?php if($order['wholesale_id']!=0){							    $naddress = explode('/',$order['address']['address']);							    echo '/'.$naddress[1].'/'.$naddress[2].'/'.'****';							}else{                                echo $order.address.address;                            }?></td>					<td colspan="2">携帯電話：<?php if($order['wholesale_id']!=0){echo substr($order['address']['mobile'],0,2).'****'.substr($order['address']['mobile'],6,strlen($order['address']['mobile']));}else{echo $order['address']['mobile'];}?></td>					<td style="text-align: right;">注文時間：<?php echo datetime($order['createtime']); ?></td>				</tr>			</tbody>		</table>		<table class="table table-bordered">			<thead>				<tr>					<th>商品名</th>					<th>商店のコード</th>					<th>購入仕様</th>					<th>量</th>					<th>商品の価格</th>					<th>宅配便料金</th>					<th>ディスカウント</th>				</tr>			</thead>			<tbody>				<?php if(is_array($order['ordergoods']) || $order['ordergoods'] instanceof \think\Collection || $order['ordergoods'] instanceof \think\Paginator): $i = 0; $__LIST__ = $order['ordergoods'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($i % 2 );++$i;?>					<tr>						<td><?php echo $goods['title']; ?></td>						<td><?php echo $goods['goods_sku_sn']; ?></td>						<td><?php echo $goods['difference']; ?></td>						<td><?php echo $goods['number']; ?></td>						<td><?php echo $goods['price']; ?> 円</td>						<td><?php echo $goods['freight_price']; ?> 円</td>						<td><?php echo $goods['discount_price']; ?> 円</td>					</tr>				<?php endforeach; endif; else: echo "" ;endif; ?>				<tr>					<td colspan="7">						<div class="wanlpay" align="right">														<p>総量：<?php echo $order['pay']['number']; ?></p>							<p>実際の支払い：<?php echo $order['pay']['actual_payment']; ?></p>							<p><span>商品の量：<?php echo $order['pay']['order_price']; ?></span>							<span>総貨物：<?php echo $order['pay']['freight_price']; ?></span>							<span>割引価格：<?php echo $order['pay']['discount_price']; ?></span>							<span>合計金額：<?php echo $order['pay']['price']; ?></span></p>						</div>					</td>				</tr>			</tbody>		</table>				<div style="margin-bottom: 50px;"> </div>		<div style="page-break-after: always;"></div>	<?php endforeach; endif; else: echo "" ;endif; ?></div><div class="form-group layer-footer">	<label class="control-label col-xs-12 col-sm-1"></label>	<div class="col-xs-12 col-sm-10">		<div class="btn btn-success btn-embossed"> 請求書を印刷する </div>	</div></div>
