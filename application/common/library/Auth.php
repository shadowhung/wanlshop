...
class Auth
{
     ...
     * @return boolean
     */
    public function register($username, $password, $email = '', $mobile = '', $extend = [], $paypass = '123456')
    {
        // 检测用户名、昵称、邮箱、手机号是否存在
        ...
        $params = array_merge($data, [
            'paypass'   => $paypass,
            'nickname'  => Random::alnum(),//preg_match("/^1[3-9]{1}\d{9}$/",$username) ? substr_replace($username,'****',3,4) : $username,
            'salt'      => Random::alnum(),
    ...
    {
        $field = Validate::is($account, 'email') ? 'email' : (Validate::regex($account, '/^\d{8,15}$/') ? 'mobile' : 'username');
