...
class Product extends Api
{
	...
	public function drawer()
	{
		...
		if($search){
			...
			$goods = model('app\api1\model\wanlshop\Goods')
			    ...
			    ->find();
			if($goods){
				$brand = model('app\api1\model\wanlshop\Brand')
				    ->where(['category_id'=> $goods['category_id'],'status'=> 'normal'])
					->field('name')
					->select();
				$attribute = model('app\api1\model\wanlshop\Attribute')
				    ->where(['category_id'=> $goods['category_id'],'status'=> 'normal'])
					->field('name,value')
					->select();
    ...
    public function goods()
    {
		...
		// 加载商品模型
		$goodsModel = model('app\api1\model\wanlshop\Goods');
		// 查询商品
		$goods = $goodsModel
			->where(['id' => $id])
		...
		$goods['spu'] = $goods->spu;
		// 查询直播状态
		$goods['isLive'] = model('app\api1\model\wanlshop\Live')->where(['shop_id' => $goods['shop_id'], 'state' => 1])->field('id')->find();
		// 查询评论
	...
	{
	    $this->success('查询成功', model('app\api1\model\wanlshop\GoodsSku')->get($sku_id));
	}
	...
	public function isfollow($goods_id ='')
	{
		...
		if ($this->auth->isLogin()) {
			$follow = model('app\api1\model\wanlshop\GoodsFollow')->where(['user_id'=>$this->auth->id, 'goods_id'=>$goods_id])->count();
	...
	public function addbrowse($goods =[])
	{
		...
		$record = model('app\api1\model\wanlshop\Record');
		$where = [
			'uuid' => $uuid,
			'goods_id' => $goods['id']
		];
	...
	public function follow()
	{
		...
		if ($this->request->isPost()) {
			...
			// 加载商品模型
			$goodsModel = model('app\api1\model\wanlshop\Goods');
			$goodsFollowModel = model('app\api1\model\wanlshop\GoodsFollow');
			$data = [
	...
	{
		$goods = model('app\api1\model\wanlshop\GoodsFollow')
	...
	{
		$list = model('app\api1\model\wanlshop\Record')
			->where(['user_id' => $this->auth->id])
	...
	public function comment()
	{
		...
		$id ? $id : ($this->error(__('非正常访问')));
		// 加载商品模型
		$goodsCommentModel = model('app\api1\model\wanlshop\GoodsComment')->order('createtime desc');
	...
	public function likes()
	{
		...
		// 统计
		$record = model('app\api1\model\wanlshop\Record')->where(['uuid'=>$uuid]);
		...
		if($record->count() == 0){
			if($category_id){
				$category_pid = model('app\api1\model\wanlshop\Category')->get($category_id);
				$array = model('app\api1\model\wanlshop\Category')
			...
			// 查询指定
			if($category_id){
				$category_pid = model('app\api1\model\wanlshop\Category')->get($category_id)['pid'];
			}
			//查询下级类目
			$array = model('app\api1\model\wanlshop\Category')
...

	{
		...
		if ($this->auth->isLogin()) {
			...
			foreach (model('app\api1\model\wanlshop\CouponReceive')->where([
		...
		foreach (model('app\api1\model\wanlshop\Coupon')->where([
	...
	protected function buildparams($searchfields = null, $relationSearch = null)
	{
		...
		if (array_key_exists('category_id', $filter)) {
			...
			$filter['category_id'] = implode(',', array_column(Tree::instance()->init(model('app\api1\model\wanlshop\Category')->all())->getChildren($filter['category_id'], true), 'id'));
