...
class Address extends Api
{
    ...
    public function getaddress()
    {
		...
		$list = model('app\api1\model\wanlshop\Address')
    ...
    public function address()
    {
        if ($this->request->isPost()) {
        	...
        	$address = new \app\api1\model\wanlshop\Address();
        	$data = $request['data'];
        	...
        	switch ($request['type']) {
				case "edit": 
					if($count <= 1){
						...
						// 单独设置默认，避免非默认消耗资源
						if($data['default'] == 1){
							$list = \app\api1\model\wanlshop\Address::all(['user_id'=>$data['user_id']]);
    ...
    public function deladdress()
    {
        if ($this->request->isPost()) {
        	...
        	if(model('app\api1\model\wanlshop\Address')->where(['id'=>$id,'user_id'=>$this->auth->id])->delete()){
